...
resource "aws_flow_log" "just-eat" {
  vpc_id          = "${aws_vpc.just-eat.id}"
  iam_role_arn    = "<iam_role_arn>"
  log_destination = "${aws_s3_bucket.just-eat.arn}"
  traffic_type    = "ALL"

  tags = {
    GeneratedBy      = "Accurics"
    ParentResourceId = "aws_vpc.just-eat"
  }
}
resource "aws_s3_bucket" "just-eat" {
  bucket        = "just-eat_flow_log_s3_bucket"
  acl           = "private"
  force_destroy = true
...
resource "aws_s3_bucket_policy" "just-eat" {
  bucket = "${aws_s3_bucket.just-eat.id}"
  ...
  policy = <<POLICY
{
  ...
  "Statement": [
    {
      "Sid": "just-eat-restrict-access-to-users-or-roles",
      "Effect": "Allow",
      "Principal": [
        {
          "AWS": [
      ...
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::${aws_s3_bucket.just-eat.id}/*"
    }
  ]
}
POLICY
