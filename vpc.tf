...
resource "aws_flow_log" "demo-for-Paul-H" {
  vpc_id          = "${aws_vpc.demo-for-Paul-H.id}"
  iam_role_arn    = "<iam_role_arn>"
  log_destination = "${aws_s3_bucket.demo-for-Paul-H.arn}"
  traffic_type    = "ALL"

  tags = {
    GeneratedBy      = "Accurics"
    ParentResourceId = "aws_vpc.demo-for-Paul-H"
  }
}
resource "aws_s3_bucket" "demo-for-Paul-H" {
  bucket        = "demo-for-Paul-H_flow_log_s3_bucket"
  acl           = "private"
  force_destroy = true
...
resource "aws_s3_bucket_policy" "demo-for-Paul-H" {
  bucket = "${aws_s3_bucket.demo-for-Paul-H.id}"
  ...
  policy = <<POLICY
{
  ...
  "Statement": [
    {
      "Sid": "demo-for-Paul-H-restrict-access-to-users-or-roles",
      "Effect": "Allow",
      "Principal": [
        {
          "AWS": [
      ...
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::${aws_s3_bucket.demo-for-Paul-H.id}/*"
    }
  ]
}
POLICY
